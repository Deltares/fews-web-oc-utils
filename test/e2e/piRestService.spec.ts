...
describe("pi rest service localhost: POST timeseries/edit", function () {
    it("post timeseries/edit", async function () {
            ...
            const provider = new PiRestService(baseUrl)
            const res = await provider.postData<DataRequestResult<unknown>>("/rest/fewspiservice/v1/timeseries/edit?timeSeriesSetIndex=42&locationId=test", JSON.stringify({
                "test": "teste2e",
            }), headers);
            const c = res.data
            expect(res.data).not.toBeNull()
...
describe("pi rest service localhost: POST timeseries in PI XML format", function () {
    it("post timeseries/edit", async function () {
            ...
            const piXmlContent = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
                ...
                "    </series>\n" +
                "</TimeSeries>    \n"
            const piXmlEncoded = "piTimeSeriesXmlContent=" + encodeURIComponent(piXmlContent);
            const requestOption = new RequestOptions()
            const provider = new PiRestService(baseUrl)
            const res = await provider.postDataWithParser<DataRequestResult<string>>("/rest/fewspiservice/v1/timeseries", requestOption, new PlainTextParser(), piXmlEncoded, headers);
            expect(res.data).not.toBeNull()
            // expect(res.data).not.toBeUndefined()
...
<Diag xmlns="http://www.wldelft.nl/fews/PI" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.wldelft.nl/fews/PI https://fewsdocs.deltares.nl/schemas/version1.0/pi-schemas/pi_diag.xsd" version="1.2">
    <line level="3" description="Import.Info: 0 time series imported, 1 time series rejected"/>
            ...
            const piJsonEncoded = "piTimeSeriesJsonContent=" + encodeURIComponent(piJsonContent);
            const requestOption = new RequestOptions()
            const provider = new PiRestService(baseUrl)
            const res = await provider.postDataWithParser<DataRequestResult<string>>("/rest/fewspiservice/v1/timeseries", requestOption, new PlainTextParser(), piJsonEncoded, headers);
